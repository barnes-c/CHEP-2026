%%%%%%%%%%%%%%%%%%%%%%%%%% Packages / Definitions %%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
\documentclass[epj]{webofc}
\setlength{\footskip}{3.60004pt}
\PassOptionsToPackage{}{xcolor}
\usepackage{xcolor}
\usepackage[varg]{txfonts}
\RequirePackage{orcidlink}
\usepackage{tikz}
\usetikzlibrary{calc, positioning, arrows.meta, fit}
\definecolor{listingbg}{RGB}{245,245,245}
\definecolor{user}{RGB}{79,129,189}
\usepackage[newfloat]{minted}

\setminted{
    bgcolor=listingbg,
    fontsize=\footnotesize,
    fontfamily=tt,
    breaklines=true,
    breakanywhere=false,
    frame=single,
    framesep=6pt,
    rulecolor=\color{user},
    linenos=true,
    firstnumber=1,
    numbersep=5pt,
    stepnumber=2,
    tabsize=2,
    obeytabs=true,
    showspaces=false,
    showtabs=false,
    autogobble=true,
    escapeinside=\%*\*),
}

\definecolor{problemred}{RGB}{140, 62, 84}   % muted red, print-safe

\tikzset{
  problemnote/.style={note, text=problemred},
  problemarrow/.style={drift, draw=problemred}
}

%%%%%%%%%%%%%%%%%%%%%%%%%% Document %%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{document}
\title{An IaC-Framework for the future of CERN's Infrastructure and Configuration Management}

\author{\firstname{Christopher} \lastname{Barnes}\orcidlink{0009-0003-1963-7992}\inst{1}\fnsep\thanks{\email{christopher.barnes@cern.ch}} \and
        \firstname{Daniel} \lastname{Juárez}\orcidlink{0009-0003-2064-6503}\inst{1}\fnsep\thanks{\email{daniel.juarez.gonzalez@cern.ch}} \and
        \firstname{Luis} \lastname{Fernández Álvarez}\orcidlink{0000-0002-9714-9850}\inst{1}\fnsep\thanks{\email{luis.fernandez.alvarez@cern.ch}} \and
        \firstname{Giacomo} \lastname{Tenaglia}\orcidlink{0009-0008-0834-2546}\inst{1}\fnsep\thanks{\email{giacomo.tenaglia@cern.ch}}
}

\institute{CERN, Esplanade des Particules 1, 1217 Meyrin, Switzerland}

\abstract{\input{chapters/abstract}}

\maketitle

\input{chapters/introduction}
\input{chapters/infrastructure}
\input{chapters/design}
\input{chapters/implementation}
\input{chapters/evaluation}
\input{chapters/conclusion}

\input{references}

\end{document}
